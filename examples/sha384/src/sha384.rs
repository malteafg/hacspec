...
pub const HASH_SIZE: usize = 384 / 8;

bytes!(Block, BLOCK_SIZE);
array!(OpTableType, 12, usize);
bytes!(Sha384Digest, HASH_SIZE);
...
pub fn hash(msg: &ByteSeq) -> Sha384Digest {
    let mut h = HASH_INIT;
    ...
    Sha384Digest::from_seq(&h.to_be_bytes().slice(0,48))
}

pub fn sha384(msg: &ByteSeq) -> Sha384Digest {
    hash(msg)
}
